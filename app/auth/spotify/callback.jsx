...
export default function SpotifyCallback() {
  ...
  const handleCallback = async () => {
    try {
      ...
      const response = await socialAccountsApi.handleSpotifyCallback(code, state, error);
      
      if (response.success) {
        setStatus('success');
        setMessage('Spotify account linked successfully!');
        setTimeout(() => router.back(), 2000);
      } else {
        setStatus('error');
        setMessage(response.message || 'Failed to link Spotify account');
        setTimeout(() => router.back(), 3000);
      }
    } catch (error) {
